...
function ReviewValue(reviewValueProps) {
  return (
    <AlignItems>
      <ReviewNumberStyled>{reviewValueProps.children}</ReviewNumberStyled>
...
export default function Review(props) {
  ...
  return (
    <ReviewStyled key={props.key}>
      ...
      >
        <ReviewValue text="最寄駅のアクセス">{ratingData.access}</ReviewValue>
        <ReviewValue text="デートスポット適性">{ratingData.date}</ReviewValue>
        <ReviewValue text="設備管理の状況">{ratingData.management}</ReviewValue>
      </Grid>
    </ReviewStyled>
  );
}
